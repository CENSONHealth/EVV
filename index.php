...
function underscoreToCamelCase( $string, $first_char_caps = false) {
        return preg_replace_callback('/_([a-z])/', create_function('$c', 'return strtoupper($c[1]);'), (($first_char_caps === true) ? ucfirst($string) : $string));
}
...
require_once('ESF.php');  //This is the engine of the whole system
...
//###########################################################################
//Primes the Jarvis Engine
$ns              = $namespace;      //save a copy, since this might change if there's no specific action but there is a default action
\Jarvis::_namespace($namespace);     //this will become the inherited namespace if necessary
\Jarvis::_controller($controller);
\Jarvis::_action($method);
...
$module          = \Jarvis::getModule($namespace);
if (!$module) {
    ...
    $core            = \Jarvis::getModule('evv');    //A reference to the core functionality held in the Core module
    ...
    if (file_exists($include)) {
        $info        = \Jarvis::getController($namespace.'/'.$controller);
        ...
        $include         = 'Code/Main/EVV/Controllers/Cache/'.$controller.'Controller.php';
        $source          = 'Code/Main/EVV/Controllers/'.$controller.'.xml';
        if (file_exists($include)) {
            ...
            $info        = \Jarvis::getController($ns.'/'.$controller);
            $recompile   = ($info['compiled'] != date("F d Y, H:i:s", filemtime($source)));
        } else if (file_exists($source)) {
            $ns          = 'core';
            $info        = \Jarvis::getController($ns.'/'.$controller);
    ...
    if ($recompile) {
        try {
            ...
            \JarvisException::standard($e,'Parameter Configuration Error','custom');
        } catch (MissingControllerXMLException $e) {
            \JarvisException::standard($e,'File Name Or Location Error','custom');
        } catch (Exception $e) {
            \JarvisException::standard($e, "Compilation Error");
        }
    }
    ...
    try {
        ...
        \JarvisException::standard($e,'Required Parameter Missing','custom');
    } catch (ValidationDatatypeException $e) {
        \JarvisException::standard($e,'Data Validation Error','custom');
    } catch (NoTriggerEventException $e) {
        \JarvisException::standard($e,'Invalid Trigger Event','custom');
    } catch (UnconfiguredException $e) {
        \JarvisException::standard($e,'Workflow Configuration Error','workflow');
    } catch (IncompleteConfigurationException $e) {
        \JarvisException::standard($e,'Workflow Component Error','workflow');
    } catch (SmartyCompilerException $e) {
        \JarvisException::standard($e,'Smarty Compiler Error','smarty');
    } catch (Exception $e) {
        \JarvisException::standard($e,"Error Encountered");
    }
}